...
import { useChangeCompletedStatusMutation, useGetBoardDataQuery } from '../BoardContainerApiSlice';
        ...
        <tr>
            ...
            > {
                row.completed ? (
                    ...
                    <MinusCircleIcon className="h-5 w-5 text-gray-400 cursor-pointer" aria-hidden="true" onClick={handleButtonClick} /> 
                )  
            }
            </td>
            ...
            <td className="px-3 py-4 text-sm text-gray-500">{new Date(row.completed_at).toLocaleDateString()}</td>
        </tr>
    );    
};
